...
| `canTxQueue` | Queue | 5 messages | File des messages CAN à envoyer |
| `tempHumQueue` | Queue | 3 éléments | Partage temp+hum vers `TaskRegroupementDonnee` |
| `irrQueue` | Queue | 3 éléments | Partage irradiance vers `TaskRegroupementDonnee` |
...
| `TaskMesureHumiditeTemperature` | 9 | 2048 | `EVENT_HUM / TEMP / ALL_TEMP_HUM` | `canTxQueue` ou `tempHumQueue` |
...
```mermaid
graph LR
    CAN_ISR["ISR OnReceiveCan\n(interruption)"]
    SER_ISR["ISR OnReceiveSerial\n(interruption)"]

    CAN_ISR -->|"SetBitsFromISR"| SEG["systemEventGroup"]
    SER_ISR -->|"SetBitsFromISR"| SerEG["serialEventGroup"]

    SEG -->|"BIT0/1/3"| TH["TaskMesure\nHumiditeTemperature"]
    SEG -->|"BIT2/4"| TI["TaskMesure\nIrradiance"]
    SEG -->|"BIT5"| TR["TaskRegroupement\nDonnee"]
    SEG -->|"BIT6"| TN["TaskEnvoi\nNumeroCarte"]
    SerEG -->|"BIT7"| TS["TaskTraitement\nMessagesSerie"]

    TH -->|"mesures indiv."| CTX["canTxQueue"]
    TH -->|"mesure groupée"| THQ["tempHumQueue"]
    TI -->|"mesures indiv."| CTX
    TI -->|"mesure groupée"| IQ["irrQueue"]
    THQ --> TR
    IQ --> TR
    TR --> CTX
    TN --> CTX
...
data[0] = humidityInt / 256
data[1] = humidityInt % 256
```

En cas d'échec de lecture du capteur AM2315 (`isnan()`), la tâche effectue des nouvelles tentatives automatiques en boucle jusqu'à obtenir une valeur valide.
...
```
Octet   : [0]   [1]   [2]   [3]   [4]   [5]
Champ   : ├─Hum×100──┤ ├─Temp×100─┤ ├─Irr×100──┤
